...
# Configuración del DataGridView (eliminar línea problemática y ajustar orden)
...
# 1. Crear menú contextual PRIMERO (antes de cualquier referencia)
...
# 2. Configurar evento de copiado DENTRO del contexto del menú
$copyMenu.Add_Click({
    if ($dgvResults.SelectedCells.Count -gt 0) {
        $value = $dgvResults.SelectedCells[0].Value
        [System.Windows.Forms.Clipboard]::SetText($value)
    }
})

# 3. Asignar menú contextual al DataGridView
$dgvResults.ContextMenuStrip = $contextMenu

# 4. Evento MouseDown CORREGIDO (sin HideSelection)
$dgvResults.Add_MouseDown({
    ...
    if ($args.Button -eq [System.Windows.Forms.MouseButtons]::Right) {
        ...
        if ($hitTest.RowIndex -ge 0 -and $hitTest.ColumnIndex -ge 0) {
            ...
            $dgvResults.Rows[$hitTest.RowIndex].Cells[$hitTest.ColumnIndex].Selected = $true
        }
    }
})
