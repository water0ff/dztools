...
function Start-BackupUploadProcess {
    ...
    try {
        # 1) Comprimir con 7-Zip y contraseña
        Write-Host "Comprimiendo respaldo..." -ForegroundColor Cyan
        & $7zipPath a -tzip -p"National09" -mem=AES256 $zipPath $BackupPath

        # 2) Crear carpeta remota en Mega.nz (si no existe)
        Write-Host "Verificando/creando carpeta remota en Mega.nz: $remoteFolder" -ForegroundColor Cyan
        $mkdirCmd = "megatools mkdir --username `"$MegaUser`" --password `"$MegaPass`" --path `"$remoteFolder`""
        $mkdirResult = cmd /c $mkdirCmd 2>&1

        # Si el código de salida NO es cero, puede ser porque la carpeta ya existiera.
        if ($LASTEXITCODE -ne 0) {
            # Filtramos el mensaje que indica “already exists” (o similar) para no mostrarlo como error bloqueante
            if ($mkdirResult -match "already exists|File exists|exists") {
                ...
                Write-Host "Advertencia al crear carpeta remota (puede que exista): $mkdirResult" -ForegroundColor Yellow
            }
        }
        else {
        ...
        # 3) Subir el ZIP a Mega.nz
        Write-Host "Subiendo a Mega.nz…" -ForegroundColor Yellow
        $uploadCmd = "megatools put `"$zipPath`" --username `"$MegaUser`" --password `"$MegaPass`" --path `"$remoteFolder/$remoteName`""
        $uploadResult = cmd /c $uploadCmd 2>&1

        if ($LASTEXITCODE -eq 0) {
            # Extraer enlace de descarga (si lo devuelve)
