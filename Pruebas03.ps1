...
$listBoxDatabases.SelectionMode = 'One'  # Selección única
$listBoxDatabases.Size = New-Object System.Drawing.Size(180,150)
$listBoxDatabases.Location = New-Object System.Drawing.Point(10,150)
$listBoxDatabases.Enabled = $false  # Inicialmente deshabilitado
...
function ConvertTo-DataTable {
    ...
    if ($InputObject -eq $null) { return $dt }
    
    $InputObject | ForEach-Object {
        $row = $dt.NewRow()
        $_.PSObject.Properties | ForEach-Object {
            if (!$dt.Columns.Contains($_.Name)) {
                $col = New-Object System.Data.DataColumn $_.Name, $_.Value.GetType()
                $dt.Columns.Add($col)
            }
            $row[$_.Name] = $_.Value
        }
...
foreach ($d in $dbs) {
    $listBoxDatabases.Items.Add($d.name)
}
            ...
            $listBoxDatabases.Enabled = $true  # Habilitar al conectar
            Write-Host "`nConectado a base de datos" -ForegroundColor Yellow
...
$btnExecute.Add_Click({
    try {
        $selectedDb = $listBoxDatabases.SelectedItem
        if (-not $selectedDb) {
            throw "Seleccione una base de datos"
        }
        
        # Ejecutar consulta en la base de datos seleccionada
        $result = Execute-SqlQuery -server $global:server -database $selectedDb -query $txtQuery.Text
        
        # Convertir resultados a DataTable y asignar al DataGridView
        $dataTable = ConvertTo-DataTable $result
        $dgvResults.DataSource = $dataTable
        
        Write-Host "`nConsulta ejecutada correctamente." -ForegroundColor Green
    }
    catch {
...
# Añadir este evento:
$listBoxDatabases.Add_SelectedIndexChanged({
    if ($listBoxDatabases.SelectedItem) {
        $lblConnectionStatus.Text = "BDD seleccionada: $($listBoxDatabases.SelectedItem)"
