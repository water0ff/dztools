...
$global:backupTimer.Add_Tick({
    # Comprobar si el job cambió de estado
    if ($global:backupJob.State -in 'Completed','Failed','Stopped') {
        # 7.1) Detener ambos timers INMEDIATAMENTE
        if ($animTimer)   { $animTimer.Stop() }
        $global:backupTimer.Stop()

        # 7.2) Recuperar resultados del job (para vaciar la cola)
        Receive-Job $global:backupJob | Out-Null

        # 7.3) Mostrar mensaje según el estado
        if ($global:backupJob.State -eq 'Completed') {
            ...
            # Abrir carpeta o preguntar por subida...
            if (Test-Path $global:tempBackupFolder) {
                ...
                Write-Host "Error | No se pudo abrir la carpeta de backup:`n`t$global:tempBackupFolder" -ForegroundColor Red
            }
        }
        ...
        # 7.4) Limpiar el job y luego cerrar el formulario de progreso
        Remove-Job $global:backupJob -Force

        # IMPORTANTE: llamar a .Close() en el hilo de UI
        if ($global:backupProgressForm.InvokeRequired) {
            # Si InvokeRequired=true, usar Invoke para asegurarnos de ejecutarlo en el hilo de UI
...
"@
    ...
    # ------------------------------------------------------------------
    # 1) Variables (deben existir en el scope global del script principal)
    ...
    try {
        ...
        # --------------------------------------------------
        # 3) Subir el archivo ZIP directamente a la raíz de Mega.nz
        # --------------------------------------------------
        Write-Host "Subiendo a Mega.nz (directo a raíz)..." -ForegroundColor Yellow

        # Sintaxis: megatools put [--username USER] [--password PASS] LOCALZIP
        $uploadCmd    = "megatools put --username `"$MegaUser`" --password `"$MegaPass`" `"$zipPath`""
        $uploadResult = cmd /c $uploadCmd 2>&1

        if ($LASTEXITCODE -eq 0) {
            # Extraer enlace de descarga si Megatools lo retorna
        ...
        # --------------------------------------------------
        # 4) Limpieza: eliminar el ZIP local
        # --------------------------------------------------
