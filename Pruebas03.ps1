            ...
            elseif ($selectedService -eq "GoogleDrive" -and -not (Test-GDriveInstalled)) {
                try {
                    ...
                    if (-not (Test-GDriveInstalled)) {
                        throw "gdrive no se instaló correctamente."
                    }
                }
                catch {
                    [System.Windows.Forms.MessageBox]::Show(
...
elseif ($selectedService -eq "GoogleDrive") {
    try {
        ...
        # ACTUALIZACIÓN: Refrescar PATH y buscar gdrive.exe
        ...
        if (-not $gdrivePath) {
            $gdriveCommand = Get-Command gdrive -ErrorAction SilentlyContinue
            if ($gdriveCommand) {
                $gdrivePath = $gdriveCommand.Source
            }
        }
        
        if (-not $gdrivePath) {
            throw "gdrive.exe no encontrado después de la instalación. Por favor reinicie la aplicación."
        }
