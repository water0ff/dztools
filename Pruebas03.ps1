                ...
                if (Test-Path $global:tempBackupFolder) {
                    ...
                    if ($uploadResponse -eq [System.Windows.Forms.DialogResult]::Yes) {
                        ...
                        try {
                            ...
                            # Paso 1: Verificar megatools
                            Update-Progress -Value 10 -Message "Verificando si megatools ya está disponible..."
                            if (-not (Get-Command megatools -ErrorAction SilentlyContinue)) {
                                Update-Progress -Value 15 -Message "megatools no está instalado. Instalando con Chocolatey..."
                                Write-Host "megatools no está instalado. Instalando con Chocolatey..." -ForegroundColor Yellow
                                
                                # Instalar Chocolatey si no existe
                                if (-not (Get-Command choco -ErrorAction SilentlyContinue)) {
                                    Update-Progress -Value 20 -Message "Instalando Chocolatey..."
                                    Set-ExecutionPolicy Bypass -Scope Process -Force
                                    [System.Net.ServicePointManager]::SecurityProtocol = [System.Net.ServicePointManager]::SecurityProtocol -bor 3072
                                    iex ((New-Object System.Net.WebClient).DownloadString('https://community.chocolatey.org/install.ps1'))
                                }
                                ...
                                Update-Progress -Value 30 -Message "megatools instalado correctamente."
                                ...
                                Update-Progress -Value 30 -Message "megatools ya está instalado. Se omite la instalación."
                                Write-Host "megatools ya está instalado. Se omite la instalación." -ForegroundColor Green
                            }

                            # Paso 2: Configurar archivo INI
                            $MegaUser = "gerardo.zermeno@nationalsoft.mx"
                            $MegaPass = "National09$#"
                            $configPath = "$env:APPDATA\megatools.ini"
                            
                            Update-Progress -Value 40 -Message "Configurando megatools..."
...
"@
                                ...
                                Update-Progress -Value 50 -Message "Configuración de megatools creada correctamente."
                                Write-Host "Configuración de megatools creada correctamente." -ForegroundColor Green
                            }
                            else {
                                Update-Progress -Value 50 -Message "Archivo de configuración ya existe. No se modifica."
                                Write-Host "Archivo de configuración ya existe. No se modifica." -ForegroundColor Yellow
                            }
                            ...
                            Update-Progress -Value 60 -Message "Comprimiendo respaldo..."
                            Write-Host "Comprimiendo respaldo..." -ForegroundColor Cyan
                            & $7zipPath a -tzip -p"National09" -mem=AES256 $zipPath $global:backupPath
                            
                            # Paso 4: Subir a Mega.nz
                            Update-Progress -Value 70 -Message "Subiendo a Mega.nz (directo a raíz)..."
                            ...
                            if ($LASTEXITCODE -eq 0) {
                                ...
                                if ($downloadLink) {
                                    ...
                                    Update-Progress -Value 100 -Message "Subida completada. Enlace copiado."
                                    Write-Host "Subida completada. Enlace copiado." -ForegroundColor Green
                                }
                                else {
                                    Update-Progress -Value 100 -Message "Subida completada, pero no se pudo extraer enlace."
                                    Write-Host "Subida completada, pero no se pudo extraer enlace." -ForegroundColor Yellow
                                }
                            }
                            else {
