...
#############BACKUP
$btnBackup.Add_Click({
    # Declarar variables en ámbito superior
    ...
    try {
        ...
        $global:backupConnection.add_InfoMessage({
            if ($_.Message -match '(\d+)% procesado') {
                $percent = [int]$matches[1]
                $progressBar.BeginInvoke([Action]{
                    $progressBar.Value = $percent
                    $lblProgress.Text = "$percent% completado - $($_.Message)"
                })
            }
        })
        ...
        $btnCancel.Add_Click({
            try {
                ...
                if ($global:backupProgressForm -ne $null) {
                    ...
                    $global:backupProgressForm.Dispose()
                }
            }
        })
        ...
        $global:backupTimer.Add_Tick({
            if ($global:backupCommand -ne $null -and $global:backupCommand.AsyncState -ne $null -and $global:backupCommand.AsyncState.IsCompleted) {
                $global:backupTimer.Stop()
                try {
                    $global:backupCommand.EndExecuteNonQuery($global:backupCommand.AsyncState)
                    [System.Windows.Forms.MessageBox]::Show("Backup completado exitosamente en:`n$backupPath", "Éxito",
                        [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Information)
                }
                catch {
                    if ($_.Exception.InnerException -notmatch "Operation cancelled by user") {
                        [System.Windows.Forms.MessageBox]::Show("Error en backup: $($_.Exception.InnerException.Message)", "Error",
        ...
        # Iniciar proceso
        $global:backupCommand.BeginExecuteNonQuery()
...
# Añadir al final del script para cerrar conexiones
