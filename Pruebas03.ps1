...
$btnBackup.Add_Click({
    ...
    # 4. Formulario y ProgressBar “ping‑pong”
    ...
    # 4.1 Establecer estilo y rangos mediante enum
    $pb.Style    = [System.Windows.Forms.ProgressBarStyle]::Continuous
    $pb.Minimum  = 0
    $pb.Maximum  = 100
    $pb.Value    = 0
    $pb.Size     = New-Object System.Drawing.Size(460,25)
    ...
    $global:backupProgressForm.Controls.AddRange(@($pb, $lbl, $btnCancel))
    $global:backupProgressForm.Show()

    # 5. Animación “ping‑pong” de la barra
    $direction = 1
    $animTimer = New-Object System.Windows.Forms.Timer
    $animTimer.Interval = 50
    $animTimer.Add_Tick({
        # Asegurarse de que la propiedad existe y es numérica
        if ($pb -and $pb.PSObject.Properties['Value']) {
            if ($pb.Value -ge $pb.Maximum) { $direction = -1 }
            elseif ($pb.Value -le $pb.Minimum) { $direction = 1 }
            # Incrementar o decrementar
            $pb.Value = [Math]::Min($pb.Maximum, [Math]::Max($pb.Minimum, $pb.Value + (5 * $direction)))
        }
    })
    $animTimer.Start()
