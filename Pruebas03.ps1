...
# URLs de descarga directa (convertidas desde Google Drive)
                ...
                if (Check-Chocolatey) {
                    foreach ($tool in $missingTools) {
                        switch ($tool) {
                            ...
                            default {
                                # En caso de que Test-UploadDependencies devolviera otra etiqueta inesperada
                    ...
                    # El usuario no autorizó la instalación de Chocolatey, por lo tanto no podemos continuar
                    $script:uploadBackup = $false
                }
            } else {
    ...
    -StartPosition ([System.Windows.Forms.FormStartPosition]::CenterScreen) -FormBorderStyle ([System.Windows.Forms.FormBorderStyle]::FixedDialog)-TopMost $true
    ...
    $global:backupTimer.Add_Tick({
        if ($global:backupJob.State -in 'Completed','Failed','Stopped') {
            ...
            if ($global:backupJob.State -eq 'Completed') {
                ...
                # Solo subir si el usuario previamente eligió subir y las dependencias estaban instaladas
                if ($script:uploadBackup) {
                    try {
                        ...
                        $cancelUploadButton = Create-Button -Text "Cancelar" -Size (New-Object System.Drawing.Size(100, 30)) -Location (New-Object System.Drawing.Point(190, 120))
...
if ($LASTEXITCODE -eq 0) {
    ...
    if ($downloadLink) {
        # Limpiar URL si tiene caracteres extraños
        $cleanLink = $downloadLink -replace '[^\x20-\x7E]', ''
        
        [System.Windows.Forms.MessageBox]::Show(
            "Backup subido exitosamente`nEnlace: $cleanLink`n(Copiado al portapeles)",
            "Éxito",
            [System.Windows.Forms.MessageBoxButtons]::OK,
            [System.Windows.Forms.MessageBoxIcon]::Information
        )
