...
function Execute-SqlQuery {
    ...
    try {
        ...
        if ($query -match "^\s*SELECT") {
            # Para SELECT: Retornar DataTable para grid Y array para consola
            ...
            # Convertir a formato legacy para consola
            $results = @()
            foreach ($row in $dataTable.Rows) {
                $hash = @{}
                foreach ($col in $dataTable.Columns) {
                    $hash[$col.ColumnName] = $row[$col]
                }
                $results += $hash
            }
            
            return @{DataTable = $dataTable; ConsoleData = $results}
        } 
        else {
            # Para otros queries: Retornar solo filas afectadas
            $command = $connection.CreateCommand()
            $command.CommandText = $query
...
$btnExecute.Add_Click({
    try {
        ...
        if ($result -is [hashtable]) {
            ...
            # Mostrar en Consola
            Show-ResultsConsole -query $query
        } 
        else {
