...
$btnBackup.Add_Click({
    ...
    try {
        ...
        try {
            ...
            [System.Windows.Forms.MessageBox]::Show(
                "La ruta seleccionada no permite escritura por la cuenta de servicio SQL.`n$($folderDialog.SelectedPath)",
                "Error de permisos", [OK], [Error]
            )
            return
        }
        ...
        # (idéntico al suyo, sin cambios en scope)

        # Temporizador y cancelación…
        # (idéntico, usando $global:backupCommand)
