...
$btnBackup.Add_Click({
    ...
    $global:animTimer.Add_Tick({
        if ($global:pb.Value -ge $global:pb.Maximum)    { $global:direction = -1 }
        elseif ($global:pb.Value -le $global:pb.Minimum) { $global:direction = 1 }
        $newVal = $global:pb.Value + (5 * $global:direction)
        $global:pb.Value = [Math]::Min($global:pb.Maximum, [Math]::Max($global:pb.Minimum, $newVal))
    })
