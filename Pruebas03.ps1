...
$dgvResults = New-Object System.Windows.Forms.DataGridView
    ...
    $dgvResults.SelectionMode = [System.Windows.Forms.DataGridViewSelectionMode]::CellSelect
        ...
        # 1. Formatear celdas NULL
        $dgvResults.Add_CellFormatting({
            param($sender, $e)
            if ($e.Value -eq [DBNull]::Value) {
                $e.Value = "(NULL)"
                $e.CellStyle.ForeColor = [System.Drawing.Color]::Gray
            }
        })
                    # 2. Menú contextual para copiar
                    $dgvResults.Add_MouseDown({
                        param($sender, $args)
                        
                        if ($args.Button -eq [System.Windows.Forms.MouseButtons]::Right) {
                            $hitTest = $dgvResults.HitTest($args.X, $args.Y)
                            if ($hitTest.RowIndex -ge 0 -and $hitTest.ColumnIndex -ge 0) {
                                # Limpiar selección anterior y seleccionar la celda actual
                                $dgvResults.ClearSelection()
                                $dgvResults.CurrentCell = $dgvResults.Rows[$hitTest.RowIndex].Cells[$hitTest.ColumnIndex]
                                $dgvResults.CurrentCell.Selected = $true
                            }
                        }
                    })
            ...
            $copyMenu.Text = "Copiar"
            $copyMenu.Add_Click({
                if ($dgvResults.CurrentCell -ne $null) {
                    $value = if ($dgvResults.CurrentCell.Value -eq "(NULL)") { $null } else { $dgvResults.CurrentCell.Value }
                    [System.Windows.Forms.Clipboard]::SetText($value)
                }
            })
            $contextMenu.Items.Add($copyMenu)
            $dgvResults.ContextMenuStrip = $contextMenu
        $dgvResults.ClipboardCopyMode = 'EnableWithoutHeaderText'
