...
# Crear el formulario
    ...
    $boldFont = New-Object System.Drawing.Font("Segoe UI", 10, [System.Drawing.FontStyle]::Bold)
                                                                                                        $version = "Alfa 250219.1240"  # Valor predeterminado para la versión
...
# Función para dividir el script SQL en lotes separados por "GO"
function Split-SqlScript {
    param (
        [string]$script
    )
    # Dividir el script en lotes usando "GO" como separador
...
# Modificar el evento del botón para manejar los lotes
        ...
        if ($databaseName) {
            ...
            try {
                ...
                if ($reader.HasRows) {
                    ...
                    [System.Windows.Forms.MessageBox]::Show("La base de datos '$databaseName' ya existe. Por favor, elija otro nombre.", "Error", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Error)
                } else {
                    $reader.Close()
                    ...
                    # Leer el archivo .dat y dividir el script en lotes
                    $sqlScript = Get-Content -Path $datFilePath -Raw -Encoding UTF8
                    Write-Host "Contenido del archivo .dat:"
                    Write-Host $sqlScript

                    $batches = Split-SqlScript -script $sqlScript

                    # Ejecutar cada lote por separado
                    foreach ($batch in $batches) {
                        if (-not [string]::IsNullOrWhiteSpace($batch)) {
                            try {
                                # Verificar si el lote contiene "CREATE FUNCTION"
                                if ($batch -match "CREATE FUNCTION") {
                                    Write-Host "Ejecutando lote con CREATE FUNCTION:"
                                    Write-Host $batch

                                    # Asegurarse de que el lote solo contenga CREATE FUNCTION
                                    if ($batch -notmatch "^\s*CREATE FUNCTION") {
                                        Write-Host "Error: CREATE FUNCTION no es la primera instrucción en el lote."
                                        [System.Windows.Forms.MessageBox]::Show("Error: CREATE FUNCTION no es la primera instrucción en el lote.", "Error", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Error)
                                        continue
                                    }
                                }

                                # Agregar "USE [databaseName]" al inicio del lote
                                $batchWithContext = "USE [$databaseName];`n$batch"
                                Write-Host "Ejecutando lote:"
                                Write-Host $batchWithContext

                                $command.CommandText = $batchWithContext
                                $command.ExecuteNonQuery()
                            } catch {
                                Write-Host "Error al ejecutar el lote SQL: $batch"
                                [System.Windows.Forms.MessageBox]::Show("Error al ejecutar el lote SQL: $($_.Exception.Message)", "Error", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Error)
                                throw
                            }
                        }
                    }
                ...
                Write-Host "Error al crear la base de datos"
                [System.Windows.Forms.MessageBox]::Show("Error al crear la base de datos: $($_.Exception.Message)", "Error", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Error)
            } finally {
