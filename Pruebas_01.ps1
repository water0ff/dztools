...
# Crear el formulario
    ...
    $boldFont = New-Object System.Drawing.Font("Segoe UI", 10, [System.Drawing.FontStyle]::Bold)
                                                                                                        $version = "Alfa 250513.1717"  # Valor predeterminado para la versión
...
# Columna 4: Estado de Adaptadores en un TextBox multilineal
$textBoxAdapterStatus = Create-TextBox `
    -Location (New-Object System.Drawing.Point(700, 1)) `
    -Size     (New-Object System.Drawing.Size(260, 200)) `
...
# Botón para refrescar la lista de adaptadores
$btnRefreshAdapters = Create-Button `
    -Text     "Actualizar adaptadores" `
    -Location (New-Object System.Drawing.Point(700, 210)) `
    -ToolTip  "Vuelve a obtener y mostrar el estado actual de cada adaptador."
# Botón para convertir todas las redes a privadas
$btnTodasRedes = Create-Button `
    -Text     "Todas las redes → Privado" `
    -Location (New-Object System.Drawing.Point(900, 210)) `
    -BackColor([System.Drawing.Color]::FromArgb(224, 224, 224)) `
    -ToolTip  "Establece todas las redes como Privadas."
...
function Refresh-AdapterStatus {
    $adapters = Get-NetAdapter | Where-Object { $_.Status -eq 'Up' }
    $profiles = Get-NetConnectionProfile
    $lines = $adapters | ForEach-Object {
        $profile = $profiles | Where-Object { $_.InterfaceIndex -eq $_.ifIndex }
        $category = if ($profile) { $profile.NetworkCategory } else { "Desconocido" }
        "- $($_.Name) : $category"
    }
    $textBoxAdapterStatus.Text = $lines -join "`r`n"
}
# Eventos de clic
$btnRefreshAdapters.Add_Click({
    Refresh-AdapterStatus
    Write-Host "Adaptadores actualizados."
})

$btnTodasRedes.Add_Click({
    # Cambiar todas las redes a Privado
