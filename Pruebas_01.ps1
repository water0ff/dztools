...
$btnCreateDatabaseFromDat.Add_Click({
    ...
    if ($result -eq [System.Windows.Forms.DialogResult]::OK) {
        ...
        $databaseName = [Microsoft.VisualBasic.Interaction]::InputBox("Ingrese el nombre de la base de datos:", "Nombre de la base de datos")

        # Validar si el usuario ingresó un nombre válido
        if (-not [string]::IsNullOrEmpty($databaseName)) {
            ...
            try {
                ...
                if ($reader.HasRows) {
                    ...
                    # Cerrar reader antes de ejecutar otro comando
                    $reader.Close()
