...
if (-not (Test-Path -LiteralPath $global:tempBackupFolder)) {
    try {
        Write-Host "Creando carpeta remota: $global:tempBackupFolder" -ForegroundColor Yellow
        New-Item -ItemType Directory -LiteralPath $global:tempBackupFolder -Force | Out-Null
...
try {
    $acl = Get-Acl -LiteralPath $global:tempBackupFolder
    ...
    if (-not $acl.Access | Where-Object { $_.IdentityReference -eq "Everyone" -and $_.FileSystemRights -band [System.Security.AccessControl.FileSystemRights]::FullControl }) {
        ...
        Set-Acl -LiteralPath $global:tempBackupFolder -AclObject $acl
