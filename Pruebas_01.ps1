                ...
                if ($global:backupJob.State -eq 'Completed') {
                    ...
                    if ($chkComprimir.Checked) {
                        ...
                        if (-not (Test-7ZipInstalled)) {
                            ...
                            try {
                                # Intentar instalar 7zip
                                if (Get-Command choco -ErrorAction SilentlyContinue) {
                                    choco install 7zip -y | Out-Null
                                    Start-Sleep -Seconds 2  # opcional, darle un momento a que termine
                                    if (-not (Test-7ZipInstalled)) {
                                        throw "7-Zip no se instaló correctamente. Ruta esperada: C:\Program Files\7-Zip\7z.exe"
                                    }
                                }
                                else {
                                ...
                                # Abortar la compresión si no se pudo instalar
                                # (quedará sin comprimir, pero el backup ya se generó)
                                goto FIN_COMPRESION
                            }
                            Update-Progress -Value 60 -Message "7-Zip instalado correctamente." -WriteConsole
                        }
                        # Si llegamos aquí, 7-Zip ya está instalado (o ya existía)
