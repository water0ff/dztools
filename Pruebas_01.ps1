...
# Llenar ListBox
# Dentro del evento $btnConnectDb.Click:
$cmbDatabases.Items.Clear()
foreach ($d in $dbs) {
    $cmbDatabases.Items.Add($d.name)
}
$cmbDatabases.Enabled = $true
$cmbDatabases.SelectedIndex = 0  # Seleccionar primer item por defecto
...
$btnExecute.Add_Click({
    try {
        ...
        if (-not $selectedDb) {
            throw "Seleccione una base de datos del ComboBox"
        }
        
        # Ejecutar query usando la base seleccionada
...
# Eliminar el evento del ListBox y agregar al ComboBox
$cmbDatabases.Add_SelectedIndexChanged({
    if ($cmbDatabases.SelectedItem) {
        $lblConnectionStatus.Text = "Conectado a: $global:server | BDD: $($cmbDatabases.SelectedItem)"
