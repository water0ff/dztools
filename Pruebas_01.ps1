...
function Show-UploadPrompt {
    ...
    # Título
    $label = New-Object System.Windows.Forms.Label
    $label.Text = "¿Desea subir el respaldo a Mega.nz después de crearlo?"
    $label.Location = New-Object System.Drawing.Point(10, 20)
    $label.AutoSize = $true
    $formShowUploadPrompt.Controls.Add($label)  # FIXED: Variable name corrected
    ...
    $formShowUploadPrompt.Controls.Add($zipPic)  # FIXED
        ...
        $btnNo = Create-Button -Text "No" -Location (New-Object System.Drawing.Point(150, 180)) -ToolTipText "Cancelar acción" -Font $defaultFont -Enabled $true
...
# ############# BACKUP #############
$btnBackup.Add_Click({
# URLs de descarga directa (convertidas desde Google Drive)
...
try {
    ...
    # Intentar obtener el directorio padre, para validar credenciales y conexión
...
# Validación: si la máquina actual NO es el host (servidor), no preguntamos por Mega ni compresión
if ($env:COMPUTERNAME -ne $machineName) {
    ...
    # Paso 1: Preguntar si se subirá el respaldo
    $uploadDecision = Show-UploadPrompt

    if ($uploadDecision -eq [System.Windows.Forms.DialogResult]::Yes) {
        # 1.a) Detectar herramientas faltantes
        $missingTools = Test-UploadDependencies

        if ($missingTools.Count -gt 0) {
            # Intentar instalar Chocolatey si es necesario (y si el usuario lo autoriza)
            if (Check-Chocolatey) {
                ...
                # 1.b) Volver a verificar si aún faltan herramientas
                $missingTools = Test-UploadDependencies
                if ($missingTools.Count -gt 0) {
                ...
                # El usuario no autorizó la instalación de Chocolatey
                $script:uploadBackup = $false
            }
        } else {
            # No faltan herramientas, puede subir normalmente
            $script:uploadBackup = $true
        }
    } else {
        $script:uploadBackup = $false
...
# Asignar permisos de escritura a "Everyone"
try {
...
"@
                    ...
                    if ($LASTEXITCODE -eq 0) {
                        # Extraer URL usando DOS métodos (mayor compatibilidad)
                        $downloadLink = $null
                        ...
                        # Método 2: Buscar en todo el texto si no se encontró
                        if (-not $downloadLink) {
                        ...
                        # Si aún no se encuentra, generar enlace explícitamente (NUEVO)
                        if (-not $downloadLink) {
                            Write-Host "No se detectó enlace en salida, generando explícitamente..."
                            $fileName = [System.IO.Path]::GetFileName($zipPath)
                            $exportCmd = "megatools export --path `/Root/$fileName` "
