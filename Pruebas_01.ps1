...
function Start-BackupUploadProcess {
    ...
    # --------------------------------------------------
    #  Variables globales definidas en el script principal
    #  (asegúrese de que existan antes de invocar la función)
    #  $MegaUser  = "gerardo.zermeno@nationalsoft.mx"
    #  $MegaPass  = "National09$#"
    # --------------------------------------------------
    ...
    $remoteFull   = "$remoteFolder/$remoteName"
    ...
    try {
        ...
        # --------------------------------------------------
        # 3) Intentar crear la carpeta remota en Mega.nz
        #    usando comando posicional para mkdir
        # --------------------------------------------------
        Write-Host "Verificando/creando carpeta remota en Mega.nz: $remoteFolder" -ForegroundColor Cyan

        # Sintaxis correcta: megatools mkdir [--username USER] [--password PASS] REMOTE_DIR
        $mkdirCmd = "megatools mkdir --username `"$MegaUser`" --password `"$MegaPass`" `"$remoteFolder`""
        $mkdirResult = cmd /c $mkdirCmd 2>&1

        if ($LASTEXITCODE -ne 0) {
            # Si falla, revisamos si es porque la carpeta ya existía
            if ($mkdirResult -match "already exists|File exists|exists") {
                Write-Host "La carpeta remota '$remoteFolder' ya existe. Continuando…" -ForegroundColor Green
            }
            else {
                Write-Host "Advertencia al crear carpeta remota: $mkdirResult" -ForegroundColor Yellow
                # No se detiene la ejecución; quizá se creó parcialmente o existe una subcarpeta
            }
        }
        else {
        ...
        #    (se pasa REMOTE_PATH como parámetro posicional)
        # --------------------------------------------------
        Write-Host "Subiendo a Mega.nz…" -ForegroundColor Yellow

        # Sintaxis correcta: megatools put [--username USER] [--password PASS] LOCALFILE REMOTE_PATH
        $uploadCmd = "megatools put --username `"$MegaUser`" --password `"$MegaPass`" `"$zipPath`" `"$remoteFull`""
        $uploadResult = cmd /c $uploadCmd 2>&1

        if ($LASTEXITCODE -eq 0) {
            # Extraer enlace de descarga, en caso de que Megatools lo devuelva
