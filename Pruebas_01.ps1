...
function Remove-SqlComments {
    ...
    # Eliminar líneas que comienzan con -- (incluyendo espacios iniciales)
    $cleanedQuery = $Query -replace "(?m)^\s*--.*\n?", ""
    # Eliminar comentarios en línea (pero no afectar URLs)
...
$btnExecute.Add_Click({
    try {
        # Limpiar la grilla antes de ejecutar
        ...
        errorQueryMessage = "Error grave:`n$($_.Exception.Message)`n`nConsulta ejecutada:`n$cleanQuery"
        # Solo escribir en consola
        Write-Host $errorQueryMessage -ForegroundColor Red
        # También limpiar la grilla al ocurrir el error
        $dgvResults.DataSource = $null
        $dgvResults.Rows.Clear()
