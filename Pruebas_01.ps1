...
############# BACKUP #############
$global:serverName       = $txtServer.Text
$global:tempBackupFolder = "\\$($global:serverName)\C$\Temp\SQLBackups"
# Crear carpeta en servidor (si no existe)
if (-not (Test-Path $global:tempBackupFolder)) {
    try {
        New-Item -Path $global:tempBackupFolder -ItemType Directory -Force | Out-Null
    }
    catch {
        [System.Windows.Forms.MessageBox]::Show(
            "No se pudo crear la carpeta de respaldo en el servidor:`n$global:tempBackupFolder",
            "Error permisos o ruta",
            [System.Windows.Forms.MessageBoxButtons]::OK,
            [System.Windows.Forms.MessageBoxIcon]::Error
        )
        return
...
$btnBackup.Add_Click({
    ...
    # 3. Validación de permisos de escritura en el Admin Share
    try {
        $testFile = Join-Path $global:tempBackupFolder "test.txt"
        [IO.File]::WriteAllText($testFile, "test")
        Remove-Item $testFile -Force
    } catch {
        [System.Windows.Forms.MessageBox]::Show(
            "Error de permisos al escribir en:`n$global:tempBackupFolder",
            "Error permisos",
            [System.Windows.Forms.MessageBoxButtons]::OK,
            [System.Windows.Forms.MessageBoxIcon]::Error
        )
    ...
    $global:backupTimer.Add_Tick({
        ...
        if ($state -in 'Completed','Failed','Stopped') {
            ...
            # Limpieza
    # Al completar exitosamente el backup:
    Receive-Job $global:backupJob | Out-Null
    [System.Windows.Forms.MessageBox]::Show(
    ...
    # Abrir carpeta remota en el servidor
    Start-Process explorer.exe $global:tempBackupFolder

    # Limpieza de job y formulario
