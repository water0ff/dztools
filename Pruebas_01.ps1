...
$btnBackup.Add_Click({
    ...
    $global:backupProgressForm = [System.Windows.Forms.Form]@{
    ...
    $global:animTimer.Add_Tick({
        ...
        $new = $pb.Value + (5 * $direction)
        $pb.Value = [Math]::Min($pb.Maximum, [Math]::Max($pb.Minimum, $new))
    })
    $global:animTimer.Start()

    # 6. Iniciar Job de backup en segundo plano
    $script = {
