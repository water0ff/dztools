...
# 2. Función para el proceso de actualización
function Invoke-ForzarActualizacion {
    Write-Host "`n=== Obteniendo componentes principales del sistema ===" -ForegroundColor Cyan
    try {
        $components = Get-WmiObject -Class Win32_Processor | Select-Object Name, Manufacturer, NumberOfCores
        $components += Get-WmiObject -Class Win32_PhysicalMemory | Select-Object Manufacturer, Capacity, Speed
        $components | Format-Table -AutoSize | Out-String | Write-Host -ForegroundColor Yellow
    }
    catch {
    ...
    try {
        ...
        Rename-Item -Path $repoPath -NewName "Repository.old" -Force
        Write-Host "Carpeta renombrada: $repoPath.old" -ForegroundColor Green
    }
    catch {
