...
"@
                        ...
                        elseif ($selectedService -eq "GoogleDrive") {
                            ...
                            # Antes de buscar gdrive.exe, comprobamos si está instalado; si no, lo instalamos con choco.
                            function Test-GDriveInstalled {
                                $commonPaths = @(
                                    "$env:ProgramData\chocolatey\bin\gdrive.exe"
                                    "${env:LocalAppData}\Programs\gdrive\gdrive.exe"
                            ...
                            try {
                                ...
                                # Buscar gdrive.exe en ubicaciones específicas
                                $gdrivePath = $null
                                $commonPaths = @(
                                    "$env:ProgramData\chocolatey\bin\gdrive.exe"
                                    "${env:LocalAppData}\Programs\gdrive\gdrive.exe"
                                ...
                                $gdrivePath = (Get-Command gdrive).Source
                                ...
                                $fileId = ($uploadResult | Select-String -Pattern 'Uploaded (\w+) at') | ForEach-Object { $_.Matches.Groups[1].Value }
                                ...
                                if ($fileId) {
                                    ...
                                    $downloadLink = ($shareResult | Select-String -Pattern 'https://drive\.google\.com[^\s]+') | ForEach-Object { $_.Matches.Value }
                                ...
                                [System.Windows.Forms.MessageBox]::Show(
                                    "Error subiendo a Google Drive: $($_.Exception.Message)",
                                    "Error",
                                    [System.Windows.Forms.MessageBoxButtons]::OK,
                                    [System.Windows.Forms.MessageBoxIcon]::Error
                                )
