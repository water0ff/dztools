...
$btnBackup.Add_Click({
    ...
    try {
        ...
        # 7. Evento de progreso de SQL
            # Dentro del evento InfoMessage de $global:backupConnection
            $global:backupConnection.Add_InfoMessage({
                ...
                if ($e.Message -match '(\d+)% procesado') {
                    ...
                    if ($pct -gt $global:lastReportedPct) {
                        ...
                        # Forzar la actualización usando Invoke
                        $global:backupProgressForm.Invoke([Action]{
                            $progressBar.Value = $pct
                            $lblProgress.Text = "$pct% - $($e.Message)"
                            $progressBar.Update()  # Forzar redibujado
                        })
                    }
                }
            })
            # Reemplazar el Timer con:
            $global:asyncResult = $global:backupCommand.BeginExecuteNonQuery($null, $null,
                [System.AsyncCallback]{
                    param($asyncResult)
                    try {
                        $global:backupCommand.EndExecuteNonQuery($asyncResult)
                        $global:backupProgressForm.Invoke([Action]{
                            if (Test-Path $global:backupPath) {
                                [System.Windows.Forms.MessageBox]::Show("Backup completado en:`n$global:backupPath")
