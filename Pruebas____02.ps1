...
$btnBackup.Add_Click({
    ...
    # 4. Crear formulario global para que sea visible en todos los scriptblocks
    ...
    $lblStatus.Text     = "Por favor espere, realizando backup..."
    ...
    # 5.1 DoWork: ejecutar backup en hilo de fondo
    $bw.Add_DoWork({
        param($sender,$e)
        ...
        # Iniciar y esperar, revisando cancelación
    ...
    # 5.2 RunWorkerCompleted: **ya corre en el hilo de UI** si se inició desde UI :contentReference[oaicite:3]{index=3}
    $bw.Add_RunWorkerCompleted({
        ...
        elseif ($e.Error) {
            [System.Windows.Forms.MessageBox]::Show(
                "Error en backup:`n$($e.Error.Message)",
                "Error",
                [System.Windows.Forms.MessageBoxButtons]::OK,
                [System.Windows.Forms.MessageBoxIcon]::Error
            )
        ...
        # Cerrar el formulario global
        $global:backupProgressForm.Close()
    })

    # 6. Cancelar desde el botón
    $btnCancel.Add_Click({
        $bw.CancelAsync()
    })

    # 7. Iniciar el worker (desde UI thread) para que RunWorkerCompleted se levante en UI :contentReference[oaicite:4]{index=4}
    $bw.RunWorkerAsync()
})
