...
#boton para ejecutar sentencias
# Modificar el evento del botón $btnExecute
        $btnExecute.Add_Click({
            try {
                ...
                if ($result -is [System.Data.DataTable]) {
                    ...
                    Write-Host "`nResultados de la consulta:" -ForegroundColor Green
                    
                    # Convertir DataTable a objetos para mostrar en consola
                    $results = @()
                    foreach ($row in $result.Rows) {
                        ...
                        $results += [PSCustomObject]$rowData
                    }
                    
                    $results | Format-Table | Out-String | Write-Host
                } else {
                    Write-Host "`nFilas afectadas: $($result)" -ForegroundColor Green
...
#Combo que dice que seleccionó una base de datos.
    $cmbDatabases.Add_SelectedIndexChanged({
        if ($cmbDatabases.SelectedItem) {
            ...
            # Ejecutar USE si es necesario (opcional)
            $queryUSE = "USE [$global:database]"
            Execute-SqlQuery -server $global:server -database "master" -query $queryUSE | Out-Null
