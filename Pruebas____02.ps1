...
function Start-BackupUploadProcess {
    ...
    try {
        # 3.1 Comprimir el archivo de respaldo con 7-Zip y contraseña
        Write-Host "Comprimiendo respaldo..." -ForegroundColor Cyan
        & $7zipPath a -tzip -p"National09" -mem=AES256 $zipPath $BackupPath

        # 3.2 Subir a Mega.nz especificando usuario y contraseña
        Write-Host "Subiendo a Mega.nz..." -ForegroundColor Yellow

        # Comando corregido: se pasan --username y --password explícitamente
        ...
        if ($LASTEXITCODE -eq 0) {
            # Extraer enlace de descarga (mega.nz devuelve la URL en stdout)
            $downloadLink = $result | Select-String -Pattern 'https://mega\.nz/file/[^\s]+' | ForEach-Object { $_.Matches.Value }
            if (-not $downloadLink) {
                Write-Host "Subida completada, pero no se pudo extraer enlace de descarga." -ForegroundColor Yellow
            }
            else {
                [System.Windows.Forms.MessageBox]::Show(
            ...
            Write-Host "Error en subida a Mega.nz: $result" -ForegroundColor Red
