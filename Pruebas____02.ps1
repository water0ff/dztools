...
function Start-BackupUploadProcess {
    ...
    $transferNowApiKey = "9595cca2-aa98-4274-9634-1b5981408a63"
    $boundary = [System.Guid]::NewGuid().ToString()

    try {
        # 1. Verificar/Instalar 7-Zip
        if (-not (Test-Path $7zipPath)) {
            Write-Host "Instalando 7-Zip..." -ForegroundColor Yellow
            $installerPath = "C:\Temp\7z-installer.exe"
            Invoke-WebRequest "https://www.7-zip.org/a/7z2301-x64.exe" -OutFile $installerPath
            Start-Process $installerPath -ArgumentList "/S" -Wait
            Remove-Item $installerPath -Force
        }

        # 2. Comprimir con contraseña
        Write-Host "Comprimiendo respaldo con cifrado AES-256..." -ForegroundColor Cyan
        $securePassword = ConvertTo-SecureString "National09" -AsPlainText -Force
        $encryptedPassword = ConvertFrom-SecureString $securePassword
        $plainPassword = [System.Runtime.InteropServices.Marshal]::PtrToStringAuto(
            [System.Runtime.InteropServices.Marshal]::SecureStringToBSTR(
                (ConvertTo-SecureString -String $encryptedPassword)
            )
        )

        & $7zipPath a -tzip -p"$plainPassword" -mem=AES256 $zipPath $BackupPath

# 3. Subir a TransferNow (método compatible con PS 5.1)
        Write-Host "Subiendo a TransferNow..." -ForegroundColor Yellow
        ...
        $headers = @{
            "X-Api-Key" = $transferNowApiKey
            "Content-Type" = "multipart/form-data; boundary=$boundary"
        }

        # Capturar detalles de la solicitud para diagnóstico
        $requestDetails = @{
            Uri = "https://api.transfernow.net/v1/files"
            Method = 'Post'
            Headers = $headers
            Body = $bodyLines
        }

        Write-Host "Detalles de la solicitud:"
        $requestDetails | Format-List | Out-String | Write-Host

        $response = Invoke-RestMethod @requestDetails -ErrorAction Stop

        # ... (código para manejar respuesta) ...
    }
    catch {
        Write-Host "Error detallado durante la subida:" -ForegroundColor Red
        ...
        if (Test-Path $zipPath) { 
            ...
            Write-Host "Archivo ZIP eliminado: $zipPath" -ForegroundColor Yellow
        }
    }
}
