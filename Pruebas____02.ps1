...
# Crear los botones utilizando la función
    ...
    $btnForzarActualizacion = Create-Button -Text "Forzar actualización de datos del sistema" -Location (New-Object System.Drawing.Point(470, 250)) `
...
function Invoke-ForzarActualizacion {
    Write-Host "`n=== Obteniendo componentes principales del sistema ===" -ForegroundColor Cyan
    try {
        ...
        $procesador | Format-List | Out-String | Write-Host
        ...
        $memoria | Format-Table -AutoSize | Out-String | Write-Host
        ...
        Write-Host "Discos duros:" -ForegroundColor Yellow
        $discos | Format-Table -AutoSize | Out-String | Write-Host
    }
    catch {
        Write-Host "Error al obtener componentes: $_" -ForegroundColor Red
    }
    ...
    try {
        ...
        # Esperar a que el servicio esté completamente operativo
    ...
    Write-Host "`n=== Volviendo a obtener componentes después de actualización ===" -ForegroundColor Cyan
    try {
        # Usar CIM en lugar de WMI para mayor confiabilidad
        $procesador = Get-CimInstance -ClassName CIM_Processor | 
            Select-Object Name, Manufacturer, NumberOfCores, MaxClockSpeed
        Write-Host "`nProcesador (actualizado):" -ForegroundColor Yellow
        $procesador | Format-List | Out-String | Write-Host

        $memoria = Get-CimInstance -ClassName CIM_PhysicalMemory | 
            Select-Object Manufacturer, @{n='CapacityGB'; e={[math]::Round($_.Capacity/1GB, 2)}}, Speed
        Write-Host "Memoria RAM (actualizada):" -ForegroundColor Yellow
        $memoria | Format-Table -AutoSize | Out-String | Write-Host

        $discos = Get-CimInstance -ClassName CIM_DiskDrive | 
            Select-Object Model, @{n='SizeGB'; e={[math]::Round($_.Size/1GB, 2)}}
        Write-Host "Discos duros (actualizados):" -ForegroundColor Yellow
        $discos | Format-Table -AutoSize | Out-String | Write-Host
    }
    catch {
        Write-Host "Error al obtener componentes actualizados: $_" -ForegroundColor Red
    }
}
##-------------------------------------------------------------------------------BOTONES#
...
$btnForzarActualizacion.Add_Click({
    Write-Host "`nIniciando actualización forzada de datos del sistema..." -ForegroundColor Cyan
    Invoke-ForzarActualizacion
    Write-Host "`nProceso de actualización completado" -ForegroundColor Green
})
#Boton para salir
    $btnExit.Add_Click({
