...
# Crear el formulario
    ...
    $boldFont = New-Object System.Drawing.Font("Segoe UI", 10, [System.Drawing.FontStyle]::Bold)
                                                                                                        $version = "Alfa 250529.1313"  # Valor predeterminado para la versión
...
# ------------------------------------------------------------
# 1. Verificar e instalar megatools solo si NO está instalado
# ------------------------------------------------------------
Write-Host "`nVerificando si megatools ya está disponible..." -ForegroundColor Cyan

if (-not (Get-Command megatools -ErrorAction SilentlyContinue)) {
    Write-Host "megatools no está instalado. Procediendo a instalar con Chocolatey..." -ForegroundColor Yellow

    # Función para instalar Chocolatey si no existe
    function Check-Chocolatey {
        if (-not (Get-Command choco -ErrorAction SilentlyContinue)) {
    ...
    if (Check-Chocolatey) {
        try {
            ...
            # Asegurar que la carpeta de choco esté en PATH (para el proceso actual y futuras sesiones)
            $newPath = "C:\ProgramData\chocolatey\bin"
            if (-not ($env:Path -split ";" | Where-Object { $_ -ieq $newPath })) {
                $env:Path += ";" + $newPath
                [Environment]::SetEnvironmentVariable("Path", $env:Path, "Machine")
            }
        }
        catch {
            Write-Host "Error al instalar megatools: $($_.Exception.Message)" -ForegroundColor Red
            return
        }
    }
}
else {
    Write-Host "megatools ya está instalado. Se omite la instalación." -ForegroundColor Green
}

# ------------------------------------------------------------
# 2. Crear (solo si no existe) el archivo de configuración de megatools
# ------------------------------------------------------------
...
try {
    if (-not (Test-Path $configPath)) {
        # Asegurarse de que la carpeta AppData\Roaming existe (normalmente existe para cada usuario)
        $configDir = Split-Path -Path $configPath -Parent
        if (-not (Test-Path $configDir)) {
            New-Item -ItemType Directory -Path $configDir -Force | Out-Null
        }

        # Crear contenido de configuración solo si no existe
        $megaConfig = @"
[Login]
Username = $MegaUser
Password = $MegaPass
"@
        ...
        # Establecer permisos: que el usuario actual tenga control total
        ...
        Write-Host "Configuración de megatools creada correctamente en '$configPath'." -ForegroundColor Green
    }
    else {
        Write-Host "El archivo de configuración de megatools ya existe. No se modifica." -ForegroundColor Yellow
    }
}
catch {
    Write-Host "Error al crear o configurar 'megatools.ini': $($_.Exception.Message)" -ForegroundColor Red
}


# ------------------------------------------------------------
# 3. Función Start-BackupUploadProcess (con comando megatools corregido)
function Start-BackupUploadProcess {
    param([string]$BackupPath)

    # --------------------------------------------------
    # 1) Variables (asegúrese de declararlas antes de llamar a la función)
    #    $MegaUser = "gerardo.zermeno@nationalsoft.mx"
    #    $MegaPass = "National09$#"
    # --------------------------------------------------
    $zipPath      = "$BackupPath.zip"
    $7zipPath     = "C:\Program Files\7-Zip\7z.exe"
    # Carpeta base y subcarpeta en Mega.nz
    $remoteBase   = "/Backups"
    $remoteFolder = "/Backups/SQL"

    try {
        # --------------------------------------------------
        # 2) Comprimir el respaldo con 7-Zip y contraseña
        # --------------------------------------------------
        Write-Host "Comprimiendo respaldo..." -ForegroundColor Cyan
        & $7zipPath a -tzip -p"National09" -mem=AES256 $zipPath $BackupPath

        # --------------------------------------------------
        # 3) Crear carpetas remotas en Mega.nz (recursivo)
        #    3.1) Intentar crear /Backups
        #    3.2) Intentar crear /Backups/SQL
        # --------------------------------------------------
        Write-Host "Verificando/creando carpeta remota en Mega.nz: $remoteBase" -ForegroundColor Cyan
        $mkdirBaseCmd = "megatools mkdir --username `"$MegaUser`" --password `"$MegaPass`" `"$remoteBase`""
        $mkdirBaseResult = cmd /c $mkdirBaseCmd 2>&1

        if ($LASTEXITCODE -ne 0) {
            if ($mkdirBaseResult -match "already exists|File exists|exists") {
                Write-Host "La carpeta remota '$remoteBase' ya existe. Continuando…" -ForegroundColor Green
            }
            else {
                Write-Host "Advertencia al crear '$remoteBase': $mkdirBaseResult" -ForegroundColor Yellow
            }
        }
        else {
            Write-Host "Carpeta remota '$remoteBase' creada con éxito." -ForegroundColor Green
        }

        Write-Host "Verificando/creando carpeta remota en Mega.nz: $remoteFolder" -ForegroundColor Cyan
        $mkdirFolderCmd = "megatools mkdir --username `"$MegaUser`" --password `"$MegaPass`" `"$remoteFolder`""
        $mkdirFolderResult = cmd /c $mkdirFolderCmd 2>&1

        if ($LASTEXITCODE -ne 0) {
            if ($mkdirFolderResult -match "already exists|File exists|exists") {
                Write-Host "La carpeta remota '$remoteFolder' ya existe. Continuando…" -ForegroundColor Green
            }
            else {
                Write-Host "Advertencia al crear '$remoteFolder': $mkdirFolderResult" -ForegroundColor Yellow
            }
        }
        else {
            Write-Host "Carpeta remota '$remoteFolder' creada con éxito." -ForegroundColor Green
        }

        # --------------------------------------------------
        # 4) Subir el archivo ZIP a Mega.nz
        #    (se usa --path para indicar carpeta remota,
        #     el nombre del ZIP se conserva igual que el local)
        # --------------------------------------------------
        Write-Host "Subiendo a Mega.nz…" -ForegroundColor Yellow
        $uploadCmd = "megatools put --username `"$MegaUser`" --password `"$MegaPass`" --path `"$remoteFolder`" `"$zipPath`""
        $uploadResult = cmd /c $uploadCmd 2>&1

        if ($LASTEXITCODE -eq 0) {
            # Extraer enlace de descarga (si Megatools lo devuelve en stdout)
            $downloadLink = $uploadResult |
                Select-String -Pattern 'https://mega\.nz/file/[^\s]+' |
                ForEach-Object { $_.Matches.Value }

            if (-not $downloadLink) {
                Write-Host "Subida completada, pero no se pudo extraer el enlace de descarga." -ForegroundColor Yellow
            }
            else {
                [System.Windows.Forms.MessageBox]::Show(
            ...
            Write-Host "Error en subida a Mega.nz: $uploadResult" -ForegroundColor Red
            throw "Error al subir a Mega.nz"
        }
    }
    catch {
        Write-Host "Error detallado durante la subida: $($_.Exception.Message)" -ForegroundColor Red
        [System.Windows.Forms.MessageBox]::Show(
        ...
        # --------------------------------------------------
        # 5) Limpieza: eliminar el ZIP local en todo caso
        # --------------------------------------------------
