...
$btnBackup.Add_Click({
    try {
        ...
        $backupName = "$selectedDb-$(Get-Date -Format 'yyyyMMdd-HHmmss').bak"
        $backupPath = Join-Path $folderDialog.SelectedPath $backupName

# Configurar formulario de progreso
# Configurar formulario de progreso (CORRECCIÓN APPLICADA AQUÍ)
        $progressForm = Create-Form -Title "Progreso de Backup" -Size (New-Object System.Drawing.Size(400, 150))
        $progressBar = New-Object System.Windows.Forms.ProgressBar
        ...
        $progressForm.Controls.AddRange(@($progressBar, $lblProgress, $btnCancel))
        $progressForm.Show()

        # Configurar conexión
        $connStr = "Server=$($global:server);Database=master;User Id=$($global:user);Password=$($global:password);"
        $conn = New-Object System.Data.SqlClient.SqlConnection($connStr)
        $conn.Open()
        
        # Configurar comando
        $cmd = $conn.CreateCommand()
        $cmd.CommandText = "BACKUP DATABASE [$selectedDb] TO DISK='$backupPath' WITH STATS=1, CHECKSUM"
        ...
        # Temporizador para verificar finalización
        ...
        $timer.Add_Tick({
            if ($asyncResult.IsCompleted) {
                $timer.Stop()
                try {
                    $cmd.EndExecuteNonQuery($asyncResult) | Out-Null
