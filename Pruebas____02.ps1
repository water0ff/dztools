...
function Execute-SqlQuery {
    param (
        [string]$server,
        [string]$database,  # Ahora se obtiene del ListBox
        [string]$query
    )
            try {
                # Cadena de conexión
...
$btnExecute.Add_Click({
    try {
        ...
        # Ejecutar en la base seleccionada
...
$listBoxDatabases.Add_SelectedIndexChanged({
    if ($listBoxDatabases.SelectedItem) {
        $lblConnectionStatus.Text = "Conectado a: $($global:server) | BDD: $($listBoxDatabases.SelectedItem)"
        $global:database = $listBoxDatabases.SelectedItem
    }
})
