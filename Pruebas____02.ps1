...
$btnBackup.Add_Click({
    ...
    $global:lastReportedPct    = 0           # ← Inicializar contador de porcentaje
    ...
    try {
        ...
        $global:backupProgressForm.Text = "Progreso de Backup"
        $global:backupProgressForm.Size = New-Object System.Drawing.Size(400,150)
        ...
        # 4.1 Establecer rango mínimo y máximo
        ...
        # 4.2 Inicializar Timer de WinForms
        $global:backupTimer = New-Object System.Windows.Forms.Timer
        $global:backupTimer.Interval = 500   # Intervalo en milisegundos
        ...
        # 7. Evento de progreso de SQL (con regex mejorado)
        $global:backupConnection.Add_InfoMessage({
            param($sender, $e)
            if ($e.Message -match '([0-9]+)%') {
                $pct = [int]$matches[1]
                if ($pct -gt $global:lastReportedPct) {
                    $global:lastReportedPct = $pct
                    $global:backupProgressForm.Invoke([Action]{
        ...
        # 8. Inicio asíncrono del Backup y arranque del Timer, con DoEvents
        ...
        # 10. Manejador del Tick: fin de backup, con DoEvents y cierre ordenado
            ...
            if ($global:asyncResult.IsCompleted) {
                ...
                try {
                    ...
                    if (Test-Path $global:backupPath) {
                        ...
                        Write-Host "Advertencia | No se encontró el archivo de backup" -ForegroundColor Yellow
                    }
                }
