...
# Crear el formulario
    ...
    $boldFont = New-Object System.Drawing.Font("Segoe UI", 10, [System.Drawing.FontStyle]::Bold)
                                                                                                        $version = "Alfa 250509.1417"  # Valor predeterminado para la versión
...
$dgvResults = New-Object System.Windows.Forms.DataGridView
    ...
    $dgvResults.Size = New-Object System.Drawing.Size(740, 280)   # Más espacio vertical
    ...
    $originalForeColor = $dgvResults.DefaultCellStyle.ForeColor
    $originalHeaderBackColor = $dgvResults.ColumnHeadersDefaultCellStyle.BackColor
    $originalAutoSizeMode = $dgvResults.AutoSizeColumnsMode
...
function Set-NetworkCategory {
        ...
        foreach ($adapter in $networkAdapters) {
            ...
            #CAMBIAR FUNCION
            $lblAdaptadorStatus = Create-Label -Text $text -Location (New-Object System.Drawing.Point(10, (530 + (30 * $index)))) -Size (New-Object System.Drawing.Size(220, 20)) -BackColor ([System.Drawing.Color]::FromArgb(255, 0, 0, 0)) -ForeColor ([System.Drawing.Color]::FromArgb(255, 255, 255, 255))
...
function Execute-SqlQuery {
    ...
    try {
        ...
        if ($query -match "(?si)^\s*(WITH|SELECT|RESTORE|UPDATE.*OUTPUT|INSERT.*OUTPUT|DELETE.*OUTPUT)") {
            ...
            foreach ($row in $dataTable.Rows) {
                ...
                foreach ($col in $columns) {
                    #$rowData[$col] = $row[$col]
                    # Convierte a string, reemplaza secuencias de espacios por uno solo, y aplica Trim()
                    $rowData[$col] = (($row[$col].ToString()) -replace '\s+', ' ').Trim()
                }
...
$btnExecute.Add_Click({
    try {
        ...
        if ($result -is [hashtable]) {
            ...
            $dgvResults.DefaultCellStyle.ForeColor = 'Blue'  # <-- Color deseado para éxito
            $dgvResults.AlternatingRowsDefaultCellStyle.BackColor = '#F0F8FF'  # Fondo alternado
            # Ajuste de columnas
            $dgvResults.AutoSizeColumnsMode = [System.Windows.Forms.DataGridViewAutoSizeColumnsMode]::None
            ...
            Write-Host "Filas afectadas por query: $result" -ForegroundColor Green
        }
    }
    catch {
        # Crear DataTable para el error
        ...
        $dgvResults.Columns[0].Width = 100   # Ancho columna Tipo
        $dgvResults.Columns[1].Width = 300   # Ancho columna Mensaje
        $dgvResults.Columns[2].Width = 200   # Ancho columna Detalle
        # Resaltar en rojo
        $dgvResults.DefaultCellStyle.ForeColor = 'Red'
        $dgvResults.ColumnHeadersDefaultCellStyle.BackColor = '#FFB3B3'
        # Mostrar tooltip con query completa
