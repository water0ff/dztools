...
$btnDetenerSync.Add_Click({
    # Determinar la ruta base para Softrestaurant 10 u 11
    $softVersion = Read-Host "Ingrese la versión de Softrestaurant (10 u 11)"
    if ($softVersion -eq "10") {
        $basePath = "C:\Program Files (x86)\Softrestaurant10"
    } elseif ($softVersion -eq "11") {
        $basePath = "C:\Program Files (x86)\Softrestaurant11"
    } else {
        Write-Host "Versión de Softrestaurant no válida." -ForegroundColor Red
        return  # Salir del script si la versión no es válida
    }

    $marketPlaceBin = Join-Path $basePath "MarketPlace\bin"

    # 1. Detener NSSync.Agent
    ...
    # 2. Detener NSSync.IntegratorWorker y NSSync.Worker
    ...
    # 3. Detener MarketPlace
    $marketPlacePath = Join-Path $marketPlaceBin "Marketplace.exe"
    Stop-Process -Name "Marketplace" -ErrorAction SilentlyContinue
    Write-Host "WO: Buscando y deteniendo... (Marketplace)"


    # 4. Remover servicios (integratorworker y nsplatformworker)
    try {
