...
# Crear el formulario
    ...
    $boldFont = New-Object System.Drawing.Font("Segoe UI", 10, [System.Drawing.FontStyle]::Bold)
                                                                                                        $version = "Alfa 250211.18266666"  # Valor predeterminado para la versión
...
$btnDetenerSync.Add_Click({
    ...
    # ... (Código para buscar versiones y crear ComboBox - sin cambios)

    # 2. Obtener la ruta base
    ...
    # 3. Detener procesos
    $nssyncAgentPath = Join-Path $marketPlaceBin "NSSync.Agent.exe"
    Stop-Process -Name "NSSync.Agent" -ErrorAction SilentlyContinue
    Write-Host "WO: Buscando y deteniendo el sync. (NSSync.Agent)"

    Stop-Process -Name "NSSync.IntegratorWorker" -ErrorAction SilentlyContinue
    Stop-Process -Name "NSSync.Worker" -ErrorAction SilentlyContinue
    Write-Host "WO: Buscando y deteniendo... (NSSync.IntegratorWorker, NSSync.Worker)"

    Stop-Process -Name "Marketplace" -ErrorAction SilentlyContinue
    ...
    try {
        if (Test-Path $marketPlaceBin) {
            ...
            # Detener procesos por nombre (¡CORRECCIÓN IMPORTANTE!)
            ...
            # Verificar si hay handles abiertos (usando Get-ChildItem)
            $handlesAbiertos = Get-ChildItem -Path $marketPlaceBin -Force | Get-Process -ErrorAction SilentlyContinue
            if ($handlesAbiertos) {
                Write-Warning "Se encontraron procesos que aún usan la carpeta bin: $($handlesAbiertos.Name)"
                # Puedes agregar aquí código adicional para intentar cerrar estos procesos,
                # o simplemente mostrar un mensaje al usuario y salir.
                # Por ejemplo:
                # throw "No se pudo renombrar la carpeta porque hay procesos en uso."
                # O:
                [System.Windows.Forms.MessageBox]::Show(
                    "No se pudo renombrar la carpeta porque hay procesos en uso. Cierre las aplicaciones manualmente e intente de nuevo.",
                    "Advertencia",
                    [System.Windows.Forms.MessageBoxButtons]::OK,
                    [System.Windows.Forms.MessageBoxIcon]::Warning
                )
