...
# ... (Código para crear el formulario principal y otros componentes - sin cambios) ...
...
$btnDetenerSync.Add_Click({
    ...
    $versions = @()
    if (Test-Path "C:\Program Files (x86)\Softrestaurant10" -or Test-Path "C:\Archivos de programa (x86)\Softrestaurant10") { $versions += "10" }
    if (Test-Path "C:\Program Files (x86)\Softrestaurant11" -or Test-Path "C:\Archivos de programa (x86)\Softrestaurant11") { $versions += "11" }
    ...
    $formVersion.ShowDialog() # Mostrar el formulario modalmente

    if (-not $softVersion) { return } # Salir si no se seleccionó ninguna versión
    ...
    # 5. Renombrar carpeta bin (SOLUCIÓN AL ERROR - OPCIÓN 2: INTENTAR VARIAS VECES)
